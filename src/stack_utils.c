...
t_bool	stack_sorted(t_stack_node *stack) //Define a function that checks if the stack is sorted in ascending order
{
	if (!stack)
		return (1);
	while (stack->next) //Loop until the end of the stack is reached
	{
		if (stack->number > stack->next->number) //Check if the current value is larger than the next node's value, indicating it is out of sort
			return (0);
...
t_stack_node	*find_min(t_stack_node *stack) //Define a function that searches a stack and returns the node with the smallest number
{
	...
	while (stack) //Loop until the end of the stack is reached
	{
		if (stack->number < min) //Check if the current node value is smaller than the smallest so far
		{
			min = stack->number; //If so, update the smallest number so far
...
t_stack_node	*find_max(t_stack_node *stack) //Define a function that searches a stack and returns the node with the biggest number
{
	...
	while (stack) //Loop until the end of the stack is reached
	{
		if (stack->number > max) //Check if the current node value is smaller than the biggest so far
		{
			max = stack->number; //If so, update the biggest number so far
