...
void	pa(t_stack_node **a, t_stack_node **b, t_bool print) //Push on top of `b`, the top `a` and print the instruction
{
	push(a, b); 
	if (!print) 
		ft_printf("pa\n");
...
void	pb(t_stack_node **b, t_stack_node **a, t_bool print) //Push on top of `a`, the top `b` and print the instruction
{
	push(b, a);
	if (!print)
		ft_printf("pb\n");
