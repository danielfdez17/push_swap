...
int	error_duplicate(t_stack_node *a, int n) //Define a function that checks for duplicate input numbers in stack `a`
{
	...
	{
		if (a->number == n) //Check if the current node's value is equal to `n`. Refer to `init_stack_a()`
			return (1);
...
void	free_stack(t_stack_node **stack) //Define a function to free a stack if there are errors
{
	...
	while (current) //As long as a node exist in the stack
	{
		...
		current->number = 0; //Assigning the node to `0` before freeing is not strictly necessary but it can help catch potential bugs such as memory-leaks and improve debugging
