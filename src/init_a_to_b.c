	...
	while (a) //As long as we have nodes in stack `a`
	{
		...
		while (current_b) //Iteratively search through all the nodes in stack `b` until the end of the stack is reached
		{
			if (current_b->number < a->number 
				&& current_b->number > best_match_index) //Check if `b` node is smaller than `a` node && bigger than the closest smaller number so far
			{
				best_match_index = current_b->number; //If so, update the value of the closest smaller number so far
